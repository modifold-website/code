...
export default function ProfileProjectFeedToolbar({ username, currentPage = 1, totalPages = 1, currentSort = "downloads", showSort = true, className = "" }) {
    ...
    const getPageButtons = () => {
        ...
        for(let i = startPage; i <= endPage; i++) {
            buttons.push(
                <Link prefetch={false} key={i} href={getProjectsHref(i)} className={`button button--size-m pagination-button ${currentPage === i ? "button--type-primary" : "button--type-secondary"}`} aria-current={currentPage === i ? "page" : undefined}>
                    {i}
                </Link>
            );
        }
    ...
    const renderPagination = () => totalPages > 1 ? (
        <div className="pagination-controls profile-project-feed__pagination">
            {currentPage === 1 ? (
                ...
                <Link prefetch={false} className="button button--size-m button--type-secondary button--icon-only" href={getProjectsHref(currentPage - 1)} aria-label={t("previous")}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <path d="m15 18-6-6 6-6"></path>
                    </svg>
                </Link>
            ...
            {currentPage === totalPages ? (
                ...
                <Link prefetch={false} className="button button--size-m button--type-secondary button--icon-only" href={getProjectsHref(currentPage + 1)} aria-label={t("next")}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <path d="m9 18 6-6-6-6"></path>
                    </svg>
                </Link>
